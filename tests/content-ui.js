...
try {
  ...
  const testProduct = { id:'gid://shopify/Product/456', title:'Test piece', tags:['kmn-test'], variants:{nodes:[testVariant],pageInfo:{hasNextPage:false}}, featuredImage:{url:'https://cdn.shopify.com/s/files/1/1012/2290/8186/files/kmn-gallery-15.webp?v=1788604493',width:1080,height:1350} };
  const testDetail = detailView(testProduct,false); document.querySelector('#fixture').append(testDetail);
  assert(testDetail.querySelector('.shop-purchase').disabled && testDetail.querySelector('.shop-purchase').textContent==='オンライン販売準備中', 'Retired test products cannot open checkout');
  assert(!testDetail.querySelector('.shop-test-guide') && storefrontConfig.testProductIds.length===0, 'Retired test checkout no longer shows test payment instructions');
  ...
  const realVariants=['Strawberry','Matcha','Chocolate'].map((title,index)=>({...testVariant,id:`gid://shopify/ProductVariant/${200+index}`,title,image:{url:`https://example.com/${index}.jpg`}}));
  const teapotDetail=detailView({...testProduct,id:'gid://shopify/Product/999',tags:[],descriptionHtml:'<h2>仕様</h2><ul><li>500 ml</li></ul><script>ignored</script>',images:{nodes:Array.from({length:9},(_,index)=>({url:`https://example.com/${index}.jpg`}))},variants:{nodes:realVariants,pageInfo:{hasNextPage:false}}},false);
  assert(teapotDetail.querySelectorAll('.shop-thumbnails button').length===9 && teapotDetail.querySelectorAll('select option').length===3,'A product exposes all nine photos and three variants');
  const variantSelect=teapotDetail.querySelector('select'); variantSelect.value=realVariants[1].id; variantSelect.dispatchEvent(new Event('change'));
  assert(teapotDetail.querySelector('.shop-detail-photo img').src===realVariants[1].image.url && teapotDetail.querySelector('.shop-purchase').disabled,'Selecting a real variant changes its photo without enabling payment');
  assert(teapotDetail.querySelector('.shop-description h2').textContent==='仕様' && teapotDetail.querySelector('.shop-description li').textContent==='500 ml' && !teapotDetail.querySelector('script') && [...teapotDetail.querySelector('.shop-detail-copy').children].indexOf(variantSelect.parentElement)<[...teapotDetail.querySelector('.shop-detail-copy').children].indexOf(teapotDetail.querySelector('.shop-description')),'Product rich text remains safe and follows the variant controls');
  ...
  assert(soldDetail.querySelector('.shop-purchase').disabled, 'Sold-out items cannot proceed to checkout');
  testDetail.remove();
  ...
  gallery.querySelector('.gallery-card button').click();
  ...
  assert(gallery.querySelector('h2').textContent==='English photo title' && gallery.querySelector('.gallery-card p').textContent==='English photo description' && gallery.querySelector('.news-tag').textContent==='Our Craft', 'English gallery uses CMS translations for titles, captions and categories');
  assert(gallery.querySelector('img').alt==='English photo title' && gallery.querySelector('.gallery-card button').getAttribute('aria-label')==='Enlarge English photo title', 'English photo alternative text and controls are translated');
  gallery.querySelector('.gallery-card button').click();
  ...
  assert(englishDetail.querySelector('.shop-description').textContent==='English product description'&&englishDetail.querySelector('.shop-purchase').textContent==='Online sales coming soon'&&englishDetail.querySelector('.shop-purchase').disabled,'English product descriptions preserve the sales preparation gate');

...
最新のトップ・商品一覧・商品詳細は `assets/layout-review.css` と `assets/shop.css` を使用し、本文・ナビ・英字ラベル・JP/ENをNoto Sans JP、主要見出しをShippori Minchoに統一しています。`tokens.css` の `--lr-*` が採用画像に基づく配置・画像枠・余白の基準です。ヘッダーのナビは15px、ショップボタンは14px・左右内側24px・右外側32px。880px以下はモバイルメニューへ切り替えます。写真の透過境界は `materials-preview.css` で扱い、商品写真は1:1の枠にcontainで配置します。以降は初期デザインシステムの説明で、異なる値は最新トークンと上記が優先です。
